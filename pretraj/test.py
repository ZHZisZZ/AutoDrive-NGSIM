...
error_regularized_adapt = 0; hard_braking_regularized_adapt = 0
...
for i in range(number_vehicles):
  ...
  hard_braking, (ds_record, _, _) = predict.predict(ego, pre, observe_frames, predict_frames, 'IDM')
  error_IDM += metrics(np.array(ds_record), np.array(groundtruth_record))
  hard_braking_IDM += hard_braking

  hard_braking, (ds_record, _, _) = predict.predict(ego, pre, observe_frames, predict_frames, 'adapt')
  error_adapt += metrics(np.array(ds_record), np.array(groundtruth_record))
  hard_braking_adapt += hard_braking

  hard_braking, (ds_record, _, _) = predict.predict(ego, pre, observe_frames, predict_frames, 'regularized_adapt')
  error_regularized_adapt += metrics(np.array(ds_record), np.array(groundtruth_record))
  hard_braking_regularized_adapt += hard_braking

  # hard_braking, (ds_record, _, _) = predict.predict(ego, pre, observe_frames, predict_frames, 'nn')
  # error_nn += metrics(np.array(ds_record), np.array(groundtruth_record))
  # hard_braking_nn += hard_braking

  hard_braking, (ds_record, _, _) = predict.predict(ego, pre, observe_frames, predict_frames, 'constantv')
...
error_regularized_adapt /= number_vehicles
...
print('error of regularized adaptation model:', error_regularized_adapt)
# print('hardbraking of adaptation model:', hard_braking_regularized_adapt)
